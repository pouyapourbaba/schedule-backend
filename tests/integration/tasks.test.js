...
describe("/api/users", () => {
    ...
    describe("GET /sum-weeks", () => {
        ...
        it("should return the array of the weekly sum for the user", async () => {
            ...
            const response = await api.get(`/api/tasks/sum-weeks`).set("x-auth-token", token1);
    ...
    describe("GET /sum-months", () => {
        ...
        it("should return the array of the monthly sum for the user", async () => {
            ...
            const response = await api.get(`/api/tasks/sum-months`).set("x-auth-token", token1);

            expect(response.status).toBe(200);
            expect(response.body[0].total).toBe(aggregate(tasks, userId1, "month", 1));
            expect(response.body[1].total).toBe(aggregate(tasks, userId1, "month", 2));
        });

        it("should return an array of the month object with total hours of zero, if no tasks were found", async () => {
            const response = await api.get("/api/tasks/sum-months").set("x-auth-token", token1);

            const yearlyTotalHours = response.body.reduce(
                (a, b) => {
                    return { total: a.total + b.total };
                },
                { total: 0 }
    ...
    describe("GET /week/:number", () => {
        ...
        it("should return an empty array, if there is no task for that week", async () => {
            const response = await api.get("/api/tasks/week/65654").set("x-auth-token", token1);
        ...
        it("should return the tasks for the week", async () => {
            ...
            const response = await api.get("/api/tasks/week/1").set("x-auth-token", token1);
    ...
    describe("GET /month/:number", () => {
        ...
        it("should return 404 if there is no task for that week", async () => {
            const response = await api.get("/api/tasks/month/65654").set("x-auth-token", token1);
        ...
        it("should return the tasks for the month", async () => {
            ...
            const response = await api.get("/api/tasks/month/1").set("x-auth-token", token1);
    ...
    describe("DELETE /:taskId", () => {
        ...
        it("should return 404 when the task does not exist in the DB or the ID of the task is not valid", async () => {
            ...
            const response = await api.delete("/api/tasks/5").set("x-auth-token", token1);
        ...
        it("should return 404 if a user wants to delete other user's task", async () => {
            ...
            const tasksFromDbStart = await api.get("/api/tasks").set("x-auth-token", token1);

            // use the ID to delete the task with other users token
            const response = await api.delete(`/api/tasks/${tasksFromDbStart.body[0]._id}`).set("x-auth-token", token2);
        ...
        it("should return 204 and delete the task from the DB", async () => {
            ...
            const response = await api.delete(`/api/tasks/${tasksFromDbStart.body[0]._id}`).set("x-auth-token", token1);

            const tasksFromDbEnd = await await api.get("/api/tasks").set("x-auth-token", token1);
    ...
    describe("PUT /:taskId", () => {
        ...
        it("should return 404 if the task is not found or the task ID is not valid", async () => {
            const response = await api.put("/api/tasks/1").set("x-auth-token", token1);
